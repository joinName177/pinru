...
五维评分全部为 5 分代表本轮审核通过，nextPrompt 和 nextPromptType 必须为空。否则根据低分维度的具体依据生成自然语言修复提示词，不需要用户填写不满意原因；规划、验证或执行方面的可改进事项也可以生成提示词，不要求 issues.kind 必须为 bug。提示词写清需要修正或补充的具体工作和预期结果，不要求提高分数，不编造缺陷、不扩展原需求。仅缺评分证据时标记 needs_evidence，列明缺项，不把未知当作缺陷。达到 10 个有效轮次后不再引导追加轮次，但确认 Bug 的修复建议仍须保留供检查。issues.kind 分别用 bug/process/evidence。
