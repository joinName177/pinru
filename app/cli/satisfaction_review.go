...
先读取 %s/SKILL.md，并读取其中的评分规则、字段规范、证据校准、自然语言要求、next-turn-guidance 和 integration-contract 参考。材料清单位于 %s。
...
评分后逐格复核：分数与理由一致、无空泛套话、没有把后补验证归给模型、没有杜撰文件或测试。润色不得改变事实、分数和问题严重性，保留 AI 评价来源。
...
nextPrompt 只针对尚未解决且有证据的真实问题，表达触发场景、当前异常及预期表现，可保留必要技术定位。仅过程不足、仅材料缺失或已经没有待修缺陷时留空；不得为了评分不足编造 Bug，不扩展原需求。达到 10 个有效轮次后不再输出追加引导。issues.kind 分别用 bug/process/evidence。
