...
五格都用完整、通顺的自然中文，禁止“以下是”“作为 AI”等前言、机械总结、模板标签、箭头、Emoji、反引号和未写完的句子。逐项比较后重写内容重复、文字重复比例过高或语义高度相似而只替换维度名和少量同义词的描述。文件名、路径、函数名、命令、参数和报错属于必要技术引用，必须原样保留为普通文本，不能为了调整文风而删除或改名。
...
在同一次评价中完成事实核对和自然表达复读，缺少上述要素就回看证据重写；不能为保留低分而补造不足。每格写成一段连贯中文，直接说明本轮做了什么、哪里存在不足以及它带来的实际结果，不写分析提纲、日志清单或审计报告。五格不能统一套句、先夸后批，也不能用“综上所述”“总体而言”“因此给X分”等结尾。禁止“以下是”“作为 AI”等AI式前言、机械总结、反引号、Markdown 标题或列表、箭头链、Emoji、勾选图标，以及“触发节点：”“实际行为：”“证据：”“业务影响：”等固定标签；需要表达前后关系时改用正常中文连接句子。逐项比较五格，不得出现两格内容重复、文字重复比例过高、语义高度相似却只替换维度名或少量同义词的情况。每句话都要完整通顺，不保留成分残缺、搭配生硬或未写完的句子。
文件名、路径、函数名、命令、参数、报错和关键数据都是必要技术引用，不得为了润色而删除、模糊或改名，也不能因为含英文或技术符号就判定为机器化表达。去掉的只是文件名等内容外层不必要的反引号和装饰符号，例如直接写 validation.go、ValidateEvaluation 和 go test ./internal/annotation。正文保留理解评分和定位问题所需的全部关键信息，只有与结论无关的冗长日志才放入 evidence。没有把后补验证归给模型，没有杜撰文件或测试。润色不得改变事实、分数、问题严重性和执行者归属，保留 AI 评价来源；不用不可靠的AI检测器分数代替逐句质量检查，也不伪造人工评价身份。
