...
func buildCustomProjectPromptDocumentPrompt(projectName string, projectProfile *promptProjectProfile) string {
	...
	sb.WriteString("难度要求：每条必须带任务难度标签，只允许 一般、困难。代码理解必须是【困难】，但困难来自阅读、梳理和判断成本，不要求修改多文件；其余 16 条只允许【一般】或【困难】，整批精确控制为【一般】5 条、【困难】12 条，不要生成【简单】或【地狱】。一般题必须带一个真实链路压力，例如前后端数据衔接、跨页面回显、边界值处理、失败提示、筛选/统计口径或状态同步；困难题必须同时包含两个以上压力点，例如多状态流转、权限边界、异常恢复、历史数据兼容、列表详情一致性、导入导出一致性、异步刷新或测试验证边界。不要只改标签不提高内容压力，也不要为了显得高级而写成大型重构或地狱任务。\n")
	...
	sb.WriteString("代码理解要求：只输出 1 条且难度固定为【困难】，聚焦梳理一个核心页面、一次提交流程或一条主要数据链路，明确最终需要回答的问题，并要求把梳理结果沉淀为 README 文档；不要要求全量风险盘点，也不要写成需要改代码或改多文件的任务。\n")
	sb.WriteString("输出格式：只返回 Markdown 正文，不要包裹代码块，不要解释生成过程。分类标题固定为 **0-1代码生成**、**Feature迭代**、**代码理解**。每条格式只保留序号、难度标签和自然正文；一般题建议 70-110 字，困难题建议 90-140 字。例如：1. 【一般】当前会员预约后到场情况不清楚，管理员无法知道实际到课率。需要补一个签到核销入口，把预约状态和到场结果记录下来，并在取消、迟到和重复核销时给出清楚反馈，方便后续查看课程运营情况。不要输出成“会员签到核销子系统：...”这类固定标题格式。\n")
