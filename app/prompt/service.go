...
func buildCustomProjectPromptDocumentPrompt(projectName string, projectProfile *promptProjectProfile) string {
	...
	sb.WriteString("难度要求：每条必须带任务难度标签，只允许 简单、一般、困难、地狱；本批任务整体降低复杂度，建议 简单约 8 条、一般约 10 条、困难约 3 条，默认不要生成地狱。单文件或沿既有模式补入口、字段、筛选、状态展示通常为简单；跨少量页面或少量接口并带一两个业务约束通常为一般；只有涉及较多模块、多状态联动且需要额外验证时才标困难。不要为了显得高级而把普通需求写成困难。\n")
	...
	sb.WriteString("内容要求：提示词必须像真实项目排期里的研发任务，包含背景、触发场景和用户可感知行为即可，交付边界点到为止；不要默认加入复杂权限、事务一致性、异步恢复、多角色协作、复杂统计口径或跨系统联动，除非项目本身明显需要。不要出现代码片段、文件路径、类名、方法名、接口名、变量名、命令或具体实现步骤。\n")
	...
	sb.WriteString("Feature迭代要求：应是在已有功能基础上补一两个能力点，说明现有流程哪里不够、扩展后解决什么摩擦；尽量控制在少量页面、少量接口或少量状态变化。\n")
	sb.WriteString("代码理解要求：只输出 1 条，聚焦梳理一个核心页面、一次提交流程或一条主要数据链路，明确最终需要回答的问题，不要要求全量风险盘点。\n")
	sb.WriteString("输出格式：只返回 Markdown 正文，不要包裹代码块，不要解释生成过程。分类标题固定为 **0-1代码生成**、**Feature迭代**、**代码理解**。每条格式只保留序号、难度标签和自然正文，建议每条 40-80 字。例如：1. 【一般】当前会员预约后到场情况不清楚，管理员无法知道实际到课率。需要补一个签到核销入口，把预约状态和到场结果记录下来，方便后续查看课程运营情况。不要输出成“会员签到核销子系统：...”这类固定标题格式。\n")
