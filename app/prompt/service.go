...
func buildCustomProjectPromptDocumentPrompt(projectName string, projectProfile *promptProjectProfile) string {
	...
	sb.WriteString("难度要求：每条必须带任务难度标签，只允许 一般、困难。代码理解固定为【一般】，聚焦阅读、梳理和判断成本，不要求修改多文件；其余 16 条只允许【一般】或【困难】，整批精确控制为【一般】4 条、【困难】12 条，不要生成【简单】或【地狱】。一般题可以带一个真实链路压力，例如前后端数据衔接、跨页面回显、边界值处理、失败提示、筛选/统计口径或状态同步；困难题必须同时包含两个以上压力点，例如多状态流转、权限边界、异常恢复、历史数据兼容、列表详情一致性、导入导出一致性、异步刷新或测试验证边界。不要只改标签不提高内容压力，也不要为了显得高级而写成大型重构或地狱任务。\n")
