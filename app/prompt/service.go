...
func (s *PromptService) GenerateTaskPromptWithContext(ctx context.Context, req GeneratePromptRequest) (*PromptGenerationResult, error) {
	...
	if err := internalprompt.ValidatePromptWritingQuality(promptText); err != nil {
		errMsg := "生成的提示词文案不符合要求：" + err.Error()
		if failErr := s.store.FailTaskPromptGeneration(task.ID, errMsg, startedAt); failErr != nil {
			return nil, fmt.Errorf(errs.FmtPromptStatusBack, errMsg, failErr)
		}
		return nil, errors.New(errMsg)
...
func buildCustomProjectPromptDocumentPrompt(projectName string, projectProfile *promptProjectProfile, requested ...internalprompt.DocumentCounts) string {
	...
	fmt.Fprintf(&sb, "数量要求：只生成 %d 条，其中 0-1代码生成 %d 条，Feature迭代 %d 条，Bug修复 %d 条；代码理解、代码测试、代码重构各固定 1 条。数量为 0 的分类不生成题目。严格按数量生成，不擅自增减。\n", counts.Total(), counts.CodeGen, counts.Feature, counts.BugFix)
	...
	sb.WriteString("Bug修复须基于当前代码中真实存在的缺陷，写清触发条件、异常表现和修复后的结果，不能为凑数量编造问题。代码测试关注实际功能和回归验证；代码重构保持业务行为不变。\n")
	...
	sb.WriteString("代码理解要求：只输出 1 条，聚焦梳理一个核心页面、一次提交流程或一条主要数据链路，明确最终需要回答的问题，并要求把梳理结果沉淀为 README 文档；不要要求全量风险盘点，也不要写成需要改代码或改多文件的任务。难度按实际理解和验证成本判断。\n")
	sb.WriteString("输出格式：只返回 Markdown 正文，不要包裹代码块，不要解释生成过程。分类标题按顺序使用 **0-1代码生成**、**Feature迭代**、**Bug修复**、**代码理解**、**代码测试**、**代码重构**，数量为 0 的分类省略。每条格式只保留序号、难度标签和自然正文，建议每条 50-100 字。例如：1. 【一般】当前会员预约后到场情况不清楚，管理员无法知道实际到课率。需要补一个签到核销入口，把预约状态和到场结果记录下来，并在取消、迟到和重复核销时给出清楚反馈，方便后续查看课程运营情况。不要输出成“会员签到核销子系统：...”这类固定标题格式。\n")
