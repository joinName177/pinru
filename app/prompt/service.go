...
func buildCustomProjectPromptDocumentPrompt(projectName string, projectProfile *promptProjectProfile) string {
	...
	sb.WriteString("难度要求：每条必须带任务难度标签，只允许 简单、一般、困难、地狱；本批建议 简单约 3 条、一般约 10 条、困难约 8 条，默认不要生成地狱。简单题保持轻量，通常是沿既有模式补入口、字段、筛选、状态展示或局部交互；一般题必须带一个真实链路压力，例如前后端数据衔接、跨页面回显、边界值处理、失败提示、筛选/统计口径或状态同步；困难题必须同时包含两个以上压力点，例如多状态流转、权限边界、异常恢复、历史数据兼容、列表详情一致性、导入导出一致性、异步刷新或测试验证边界。不要只改标签不提高内容压力，也不要为了显得高级而写成大型重构或地狱任务。\n")
	...
	sb.WriteString("内容要求：提示词必须像真实项目排期里的研发任务，包含背景、触发场景、用户可感知行为和必要交付边界。简单题可以点到为止；一般和困难题不能只写一个入口或一个按钮，必须让人看出需要顺着数据流、状态流或用户操作闭环做完整交付。可以根据项目真实场景自然加入权限、异常恢复、多角色协作、统计口径、异步刷新、历史数据兼容或测试边界，但不要脱离项目强行拔高成跨系统架构改造。不要出现代码片段、文件路径、类名、方法名、接口名、变量名、命令或具体实现步骤。\n")
	...
	sb.WriteString("Feature迭代要求：应是在已有功能基础上补能力，说明现有流程哪里不够、扩展后解决什么摩擦。简单题可以控制在少量页面或少量状态变化；一般题要体现上游输入和下游展示或处理结果的衔接；困难题要体现规则、状态、异常或多角色视角中的至少两个交付边界。\n")
	sb.WriteString("代码理解要求：只输出 1 条，聚焦梳理一个核心页面、一次提交流程或一条主要数据链路，明确最终需要回答的问题，不要要求全量风险盘点。\n")
	sb.WriteString("输出格式：只返回 Markdown 正文，不要包裹代码块，不要解释生成过程。分类标题固定为 **0-1代码生成**、**Feature迭代**、**代码理解**。每条格式只保留序号、难度标签和自然正文；简单题建议 40-80 字，一般题建议 70-110 字，困难题建议 90-140 字。例如：1. 【一般】当前会员预约后到场情况不清楚，管理员无法知道实际到课率。需要补一个签到核销入口，把预约状态和到场结果记录下来，并在取消、迟到和重复核销时给出清楚反馈，方便后续查看课程运营情况。不要输出成“会员签到核销子系统：...”这类固定标题格式。\n")
