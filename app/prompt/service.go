...
func buildCustomProjectPromptDocumentPrompt(projectName string, projectProfile *promptProjectProfile, requested ...internalprompt.DocumentCounts) string {
	...
	sb.WriteString("输出格式：只返回 Markdown 正文，不要包裹代码块，不要解释生成过程。分类标题按顺序使用 **0-1代码生成**、**Feature迭代**、**Bug修复**、**代码理解**、**代码测试**、**代码重构**，数量为 0 的分类省略。每条格式只保留序号、难度标签和自然正文，建议每条 80-180 字。例如：1. 【一般】当前会员预约后到场情况不清楚，管理员无法知道实际到课率。需要补一个签到核销入口，把预约状态和到场结果记录下来，并在取消、迟到和重复核销时给出清楚反馈，方便后续查看课程运营情况。不要输出成“会员签到核销子系统：...”这类固定标题格式。\n")
