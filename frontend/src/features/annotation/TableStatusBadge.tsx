...
export function TableStatusBadge({ progress }: { progress?: TableProgress }) {
  ...
  const labels = {pending:'排队中',running:progress.message?.includes('正在采集')?'采集中':'制表中',error:'制表失败',cancelled:'已取消',ready:'已制表',partial:'部分完成',needs_evidence:'待补证据',stale:'待重新复审',captured:'待复审',empty:'待采集'};
  return (
    <span
      title={`${progress.error || progress.message || ''} 已复审 ${progress.reviewed ?? progress.prepared}/${progress.total} 轮，可导出 ${progress.prepared} 轮；不代表五维满分或功能无缺陷`}
